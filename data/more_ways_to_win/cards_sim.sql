CREATE TEMP TABLE all_cards_bets 
DISTSTYLE KEY 
...
(
        ...
        where
                pp.ramp_selection_id IN (SELECT ramp_selection_id FROM all_cards_bets)
        ...
        where
                1=1
                AND bf.ramp_selection_id IN (SELECT ramp_selection_id FROM all_cards_bets)
        ...
        where
                1=1
                AND sbg.ramp_selection_id IN (SELECT ramp_selection_id FROM all_cards_bets)
...
WHERE
        1=1
    AND s.ramp_selection_id in (SELECT ramp_selection_id FROM all_cards_bets)
